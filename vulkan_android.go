...
func (x *AndroidSurfaceCreateInfo) PassRef() (*C.VkAndroidSurfaceCreateInfoKHR, *cgoAllocMap) {
	...
	refeca5c35c.window, cwindow_allocs = *(**C.struct_ANativeWindow)(unsafe.Pointer(&x.Window)), cgoAllocsUnknown
...
// Ref returns a reference to C object as it is.
func (x *ANativeWindow) Ref() *C.struct_ANativeWindow {
	if x == nil {
		return nil
	}
	return (*C.struct_ANativeWindow)(unsafe.Pointer(x))
}
...
func NewANativeWindow() *ANativeWindow {
	return (*ANativeWindow)(allocStruct_ANativeWindowMemory(1))
}

// allocStruct_ANativeWindowMemory allocates memory for type C.struct_ANativeWindow in C.
// The caller is responsible for freeing the this memory via C.free.
func allocStruct_ANativeWindowMemory(n int) unsafe.Pointer {
	mem, err := C.calloc(C.size_t(n), (C.size_t)(sizeOfStruct_ANativeWindowValue))
	if mem == nil {
		panic(fmt.Sprintln("memory alloc error: ", err))
	}
	return mem
...
const sizeOfStruct_ANativeWindowValue = unsafe.Sizeof([1]C.struct_ANativeWindow{})

// PassRef returns a reference to C object as it is or allocates a new C object of this type.
func (x *ANativeWindow) PassRef() *C.struct_ANativeWindow {
	if x == nil {
		x = (*ANativeWindow)(allocStruct_ANativeWindowMemory(1))
	}
	return (*C.struct_ANativeWindow)(unsafe.Pointer(x))
}
